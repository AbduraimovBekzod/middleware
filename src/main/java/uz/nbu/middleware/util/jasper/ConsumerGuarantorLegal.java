    ...
    public ByteArrayOutputStream consumer_guarantor_legal(JSONObject requestObj, Integer lang) throws JRException, IOException {
        ...
        for(int i = 1; i <= 3; i++) {
            JasperReport jasperReport = lang == 0 ?
                    JasperCompileManager.compileReport(dir + "reports/consumer_guarantor_legal/rus/consumer_guarantor_legal_" + i + ".jrxml"):
                    JasperCompileManager.compileReport(dir + "reports/consumer_guarantor_legal/uzb/consumer_guarantor_legal_" + i + ".jrxml");
            ...
            try (ByteArrayInputStream is = new ByteArrayInputStream(str.getBytes())) {
                ...
                JasperExportManager.exportReportToPdfFile(jasperPrint, "output/consumer_guarantor_legal_" + i + ".pdf");
            }
        }

        PDFMergerUtility ut = new PDFMergerUtility();
        for (int i = 1; i <= 3; i++) {
            ut.addSource("output/consumer_guarantor_legal_" + i + ".pdf");
        }
        ut.setDestinationFileName("output/consumer_guarantor_legal.pdf");//merge pdf name
        ...
        for (int i = 1; i <= 3; i++) {
            new File("output/consumer_guarantor_legal_" + i + ".pdf").delete();
        }
